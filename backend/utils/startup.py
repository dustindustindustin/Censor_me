...
Uses asyncio.to_thread for synchronous model loads so the event loop stays responsive
and /system/status can be polled while models are initializing.
"""
...
async def initialize_models(gpu_info, state=None) -> None:
    """
    Verify required system dependencies and warm up ML models.
    Raises StartupError if a hard requirement is not met.

    Optional `state` (app.state) receives stage updates so /system/status
    can report granular progress while models load.
    ...
    _check_ffmpeg()
    if state:
        state.init_stage = "loading_ocr"
    await _init_easyocr(gpu_info.gpu_available_for_ocr)
    if state:
        state.init_stage = "loading_nlp"
...
async def _init_easyocr(use_gpu: bool) -> None:
    """Initialize EasyOCR reader in a thread pool (downloads models on first run ~100MB)."""
    try:
        ...
        await asyncio.to_thread(_get_reader, use_gpu)
...
async def _init_presidio() -> None:
    """Initialize Microsoft Presidio + spaCy NER model in a thread pool."""
    try:
        logger.info("Initializing Presidio analyzer…")
        await asyncio.to_thread(_load_presidio_sync)
